check_inchikey <- function(inchikey) {
  ...
  if (typeof(inchikey) != "character") {
    stop("The \"inchikey\" should be a 25- or 27-character string.", call. = FALSE)
  }
  
  if (length(unlist(strsplit(inchikey, split = "-"))) != 3L && length(unlist(strsplit(inchikey, split = "-"))) != 2L) {
    stop("The provided \"inchikey\" must be hyphen-divided into two (non-standard) or three (standard) parts.", call. = FALSE)
  }
  
  if (length(unlist(strsplit(inchikey, split = "-"))) == 3L && nchar(inchikey) != 27L) {
    stop("The provided (standard) \"inchikey\" is not a 27-character string.", call. = FALSE)
  }
  
  if (length(unlist(strsplit(inchikey, split = "-"))) == 2L && nchar(inchikey) != 25L) {
    stop("The provided (non-standard) \"inchikey\" is not a 25-character string.", call. = FALSE)
  }
