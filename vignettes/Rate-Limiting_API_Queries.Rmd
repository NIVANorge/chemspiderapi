...
```{r,eval=FALSE}
## The sleep_post_inchickey() function will automatically "sleep" for four seconds before any other function is called.
## This means it will run each query with a four second sleep in between, resulting in 15 queries per minute.
sleep_post_inchikey <- function(...) {
  post_inchikey(...)
  Sys.sleep(4)
}

## sapply() can be used in this context because the result of each iteration is a single vector.
demo_chemicals$queryId <- sapply(X = demo_chemicals$InChIKey, FUN = function(x) sleep_post_inchikey(inchikey = x, apikey = apikey))

demo_chemicals
```


### A `ratelimitr` examole
