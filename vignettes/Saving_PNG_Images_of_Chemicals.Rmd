...
Before downloading an example image for caffein (ChemSpider ID 2424), the `png` package needs to be installed.
...
caffein_png <- png::readPNG(caffein_image_raw)
```

### Saving the .png image

```{r,eval=FALSE}
png::writePNG(caffein_png, target = "caffein.png")
```
